...
MooreGridBscTrackingModifier<DIM>::MooreGridBscTrackingModifier()
...
template<unsigned DIM>
void MooreGridBscTrackingModifier<DIM>::UpdateAtEndOfTimeStep(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
    UpdateCellData(rCellPopulation);
}

template<unsigned DIM>
void MooreGridBscTrackingModifier<DIM>::SetupSolve(AbstractCellPopulation<DIM,DIM>& rCellPopulation, std::string outputDirectory)
{
    /*
...
template<unsigned DIM>
void MooreGridBscTrackingModifier<DIM>::UpdateCellData(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
    ...
    //connectivity radius
        static double Connectivity_radius = sqrt(2)+1e-3;
    ...
    {   
        ...
        neighbour_indices = static_cast<NodeBasedCellPopulation<DIM>*>(&rCellPopulation)->GetNodesWithinNeighbourhoodRadius(cell_index,Connectivity_radius);
            ...
            /*
            *this needs to be changed if you change the geometry from a Moore grid
...
template<unsigned DIM>
void MooreGridBscTrackingModifier<DIM>::OutputSimulationModifierParameters(out_stream& rParamsFile)
{
...
template class MooreGridBscTrackingModifier<1>;
template class MooreGridBscTrackingModifier<2>;
template class MooreGridBscTrackingModifier<3>;

// Serialization for Boost >= 1.36
#include "SerializationExportWrapperForCpp.hpp"
EXPORT_TEMPLATE_CLASS_SAME_DIMS(MooreGridBscTrackingModifier)
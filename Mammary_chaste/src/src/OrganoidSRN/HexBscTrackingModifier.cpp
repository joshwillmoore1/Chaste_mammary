...
HexBscTrackingModifier<DIM>::HexBscTrackingModifier()
...
template<unsigned DIM>
void HexBscTrackingModifier<DIM>::UpdateAtEndOfTimeStep(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
    UpdateCellData(rCellPopulation);
}

template<unsigned DIM>
void HexBscTrackingModifier<DIM>::SetupSolve(AbstractCellPopulation<DIM,DIM>& rCellPopulation, std::string outputDirectory)
{
    /*
...
template<unsigned DIM>
void HexBscTrackingModifier<DIM>::UpdateCellData(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
    ...
    //connectivity radius
        static double Connectivity_radius = 1+1e-2;
    ...
    {   
        ...
        neighbour_indices = static_cast<NodeBasedCellPopulation<DIM>*>(&rCellPopulation)->GetNodesWithinNeighbourhoodRadius(cell_index,Connectivity_radius);
            /*
            *this needs to be changed if you change the geometry from a hex grid
...
template<unsigned DIM>
void HexBscTrackingModifier<DIM>::OutputSimulationModifierParameters(out_stream& rParamsFile)
{
...
template class HexBscTrackingModifier<1>;
template class HexBscTrackingModifier<2>;
template class HexBscTrackingModifier<3>;
